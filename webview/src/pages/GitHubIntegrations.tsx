...
export const GitHubIntegrations: React.FC = () => {
  ...
  const fetchIntegrations = async () => {
    try {
      const data = await githubAPI.getIntegrations(
  ...
  const connectRepo = async (repo: GitHubRepo) => {
    try {
      await githubAPI.connectRepo(currentProject!.id.toString(), {
  ...
  const disconnectRepo = async (integrationId: string) => {
    try {
      await githubAPI.disconnectRepo(
        currentProject!.id.toString(),
        integrationId
      );
      await fetchIntegrations();
  ...
  const syncRepos = async () => {
    try {
      setSyncing(true);
      await githubAPI.syncRepos(currentProject!.id.toString());
      await fetchIntegrations();
    } catch (err: any) {
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center py-6">
          ...
          <div className="flex items-center">
            ...
            <h1 className="text-3xl font-bold text-gray-900">
              GitHub Integrations - {currentProject.name}
            </h1>
          </div>
        </div>
      </header>
              ...
              <div>
                ...
                {loading ? (
                  ...
                  <div className="grid grid-cols-1 gap-4">
                    ...
                    {repos.length === 0 && (
                      <p className="text-gray-500">
                        No repositories found. Make sure you've granted access
