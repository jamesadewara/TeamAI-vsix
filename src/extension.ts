...
export function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        vscode.window.registerWebviewViewProvider('teamai-chat', sidebarProvider, {
            webviewOptions: {
                retainContextWhenHidden: true
            }
        })
...
class TeamAISidebarProvider implements vscode.WebviewViewProvider {
...
function getReactWebviewContent(webview: vscode.Webview, extensionUri: vscode.Uri): string {
    ...
    try {
        ...
        htmlContent = htmlContent.replace(
            /(href|src)="((?!https?:\/\/|data:|vscode-webview:)[^"]*)"/g,
            (match, attr, resourcePath) => {
...
function getErrorHTML(error: any, expectedPath: string): string {
    return `<!DOCTYPE html>
        <html>
        <head>
            <meta charset="UTF-8">
